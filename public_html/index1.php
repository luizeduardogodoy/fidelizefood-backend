...
if($idx->getPostResponse("req") == "carimbo"){
	...
	try{
		...
		if($cam_qtde > $qtde->fields("qtde")){
			...
			if(!$jaExisteEsteCarimbo->EOF && $idx->getPostResponse("permiteMaisDeUm") == "N"){
...
if($idx->getPostResponse("req") == "registraOneSignal"){


	$usu = new Usuario();
	if($usu->Load("idusuario =  " . $idx->getPostResponse("UsuarioId"))){

		$usu->idnotificacao = $idx->getPostResponse("userIdOneSignal");
		$usu->Save();


		print json_encode(["dados" => "ok"]);
